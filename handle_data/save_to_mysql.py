...
try:
    db = sqlsoup.SQLSoup(SURL, session=Session)
    ...
    def insert_new(self, infodata):
        ...
        while True:
            try:
                if self.table.filter_by(to_server=to_server, methods=methods, registerAppNo=registerAppNo,
                    ...
                    # 已存在的记录直接更新
                    self.table.filter_by(to_server=to_server, methods=methods, registerAppNo=registerAppNo,
                                         customer_id=customer_id).update(infodata)
        ...
        if to_server == 'http://yct.sh.gov.cn/bizhallnz_yctnew/apply/save_info':
            ...
            try:
                ...
                self.table.filter_by(to_server=investor_url, registerAppNo=registerAppNo, pageName='gdform').update(
                    update_info)
    ...
    def del_set(self, infodata):
        ...
        try:
            ...
            if 'http://yct.sh.gov.cn/bizhallnz_yctnew/apply/investor/ajax/delete' in to_server:
                the_set = self.table.filter_by(pageName=pageName, registerAppNo=registerAppNo,
